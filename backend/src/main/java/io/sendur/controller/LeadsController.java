...
/**
 * TODO: implement Java Bean Validation on the Entities (i.e., not null, size, email format etc.,)
 * implement GlobalExceptionHandler {@link RestControllerAdvice} class to handle.
 */
@RestController
...
public class LeadsController {
    ...
    public ResponseEntity<?> getLeadByBusinessName(@PathVariable String businessName, Authentication authentication) {
        if (authService.isNotAuthorized(authentication)) {
            return authService.forbiddenResponse();
        }
        Lead qualifiedLead = leadService.getLeadByBusinessName(businessName);
        if (ObjectUtils.isNotEmpty(qualifiedLead)) {
            return ResponseEntity.status(HttpStatus.OK)
                    .lastModified(Instant.now().toEpochMilli())
                    .body(qualifiedLead);
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("Lead not Found for business: " + businessName);
    }

    @PreAuthorize("hasAuthority('OIDC_USER')")
    @GetMapping("/{city}")
    public ResponseEntity<?> getLeadsByCity(@PathVariable String city, Authentication authentication) {
        if (authService.isNotAuthorized(authentication)) {
            return authService.forbiddenResponse();
        }
        List<Lead> qualifiedLeads = leadService.getLeadsByCity(city);
        if (ObjectUtils.isNotEmpty(qualifiedLeads)) {
            return ResponseEntity.status(HttpStatus.OK)
                    .lastModified(Instant.now().toEpochMilli())
                    .body(qualifiedLeads);
        }
        return ResponseEntity.status(HttpStatus.NOT_FOUND).body("No leads found in city: " + city);
    }
